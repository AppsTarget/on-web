...
class PedidoController extends Controller
{
    ...
    public function salvar(Request $request) {
        ...
        foreach ($request->formas_pag as $forma_pag) {
            ...
            for ($i = 0; $i < $forma_pag->parcela; $i++) {
                ...
                $aux_vencimento = new \DateTime;
                $primeiro_vencimento = $aux_vencimento->createFromFormat('d/m/Y', $forma_pag->data_vencimento);
                ...
                $pedido_parcela->vencimento = date('Y-m-d', strtotime($primeiro_vencimento->format('Y-m-d') . ' + ' . ($dias_entre_parcela * $i) . ' days'));
                $pedido_parcela->save();
        ...
        foreach($request->planos as $plano){
            ...
            if (TabelaPrecos::find($plano->id_plano)->max_atv_semana != 0) $quantidade_total = intval((($intervalo->days/7) * intval(TabelaPrecos::find($plano->id_plano)->max_atv_semana)));
            ...
            $pedido_planos->qtd_total       = $quantidade_total * $plano->qtd;
            $pedido_planos->qtd_original    = $quantidade_total; //(TabelaPrecos::find($plano->id_plano)->max_atv);
            $pedido_planos->valor           = $plano->valor;
            $pedido_planos->valor_original  = $plano->valor_original;
            $pedido_planos->descr = TabelaPrecos::find($plano->id_plano)->descr;
            $pedido_planos->save();
        ...
        if ($request->enc_id_de > 0) {
            ...
            $data->id_solicitacao = $request->enc_sol;
            if ($request->enc_para > 0) $data->id_para = $request->enc_para;
            $data->save();
    ...
    public function atividades_por_pessoa($id_paciente) {
            ...
            if ($pedidos == '()'){
                ...
                $max_atv = DB::table('pedido_planos')
                       ->select(DB::raw('SUM(pedido_planos.qtd_total) AS total'))
    ...
    public function imprimir($id, $antigo) {
        try {
            ...
            // $planos = DB::select(DB::raw("
            //     SELECT
            //         tabela_precos.vigencia,
            //         pedido_planos.valor,
            //         SUM (
            //             CASE
            //                 WHEN (agenda.id IS NOT NULL AND agenda.status IN ('F', 'A') AND agenda.lixeira = 0) THEN 1
            //                 ELSE 0
            //             END
            //         ) AS atv_consu,
            //         pedido_planos.qtd_total,
            //         CASE
            //             WHEN pedido_planos.descr IS NULL THEN tabela_precos.descr
            //             ELSE pedido_planos.descr
            //         END AS descr
            ...
            $planos = DB::table('pedido_planos')
                      ->select('tabela_precos.vigencia AS vigencia',
                               ...
                               DB::raw("SUM(".
                                ...
                                DB::raw("pedido_planos.qtd_total AS max_atv"),           
                               //DB::raw("GREATEST(pedido_planos.qtd_total, pedido_planos.qtd_original) AS max_atv"),
                               DB::raw("(GREATEST(pedido_planos.qtd_total, pedido_planos.qtd_original) - LEAST(pedido_planos.qtd_total, pedido_planos.qtd_original)) AS convertidas"),
                               DB::raw("CASE WHEN (pedido_planos.descr is null) THEN tabela_precos.descr ELSE pedido_planos.descr END AS descr"))
        ...
        if ($antigo == 0) {
            $pedidos = DB::select(
                DB::raw("
                    select
                        ...
                        pedido.status,
                        pedido.assinado,
                        IFNULL(IFNULL(contrato.signed_file, contrato.original_file), 'N') AS signed_url
                    from
                        pedido
            ...
            foreach($pedidos AS $pedido){
                $total = DB::table('pedido_planos')
                            ->selectRaw("SUM(pedido_planos.qtd_total) as total")
    ...
    public function listar_contratos_pessoa($id, $bEdita, $data) {
        ...
        $pedidos = DB::select(DB::raw("
            ...
            FROM (
            	SELECT 
		            GROUP_CONCAT(DISTINCT aux.descr) AS descr, 
		            pedido.id, 
		            pedido.data,
		            MAX(aux.max_atv_semana) as max_atv_semana,
		            (
			            SUM(pedido_planos.qtd_total)
			            -
			            (SUM(
                ...
                LEFT JOIN (
                    SELECT
                        ...
                        CASE 
                            WHEN (pp.descr is null) THEN tabela_precos.descr 
                            ELSE pp.descr 
                        END AS descr
                    FROM tabela_precos
                    LEFT JOIN (
                        ...
                        FROM pedido_planos
                        GROUP BY id_plano
                    ) AS pp ON tabela_precos.id = pp.id_plano 
                ) AS aux ON aux.id_plano = pedido_planos.id_plano
	
	            LEFT JOIN agenda
		            ON agenda.id_pedido = pedido_planos.id_pedido AND agenda.id_tabela_preco = pedido_planos.id_plano
		
                WHERE pedido.id_paciente = ".$id."
                  AND pedido.data_validade >= '".$data."'
                  AND pedido.lixeira = 0
                  AND pedido.tipo_contrato <> 'P' 
        ...
        foreach ($pedidos AS $pedido) {
            $aux_planos = DB::table("pedido_planos")
                            ->select("tabela_precos.descr     AS descr",
                                    ...
                                    DB::raw('(pedido_planos.qtd_total) AS agendaveis'),
                                    DB::raw("SUM(".
    ...
    public function listar_planos_pedido(Request $request) {
        return DB::table("pedido_planos")
                        ->select(DB::raw("CASE WHEN (pedido_planos.descr is null) THEN tabela_precos.descr ELSE pedido_planos.descr END AS descr"),
                                 "tabela_precos.id        AS id",
                                 "pedido_planos.id_pedido AS id_contrato",
                                 DB::raw('(pedido_planos.qtd_total) AS agendaveis'),
                                 DB::raw("SUM(".
    ...
    public function congelar_pedido(Request $request){
        ...
        $status->status = 'S';
        $status->save();
        $congelamento = new Congelamentos;
        $congelamento->id_pedido = $pedido->id;
        $congelamento->data = $pedido->data_congelamento;
        $congelamento->acao = 'C';
        $congelamento->save();
            ...
            case 0:
                $data->planos = DB::table('pedido_planos')
                                ->select('pedido_planos.id AS id_plano',
                                        ...
                                        DB::raw('(pedido_planos.valor/(pedido_planos.qtd_total)) AS valor_und'),
                                        DB::raw('pedido_planos.valor AS valor_total'),
                                        DB::raw('(pedido_planos.qtd_total) AS qtde'),
    ...
    public function log($id) {
        ...
        $consulta = DB::select(DB::raw("
            SELECT
                id_plano,
                pedido.data,
                IFNULL(tabela_precos.id, 0) AS tab_preco
        ...
        for ($i = 0; $i < sizeof($consulta); $i++) {
            $planos[$i] = new \stdClass;
            if (intval($consulta[$i]->tab_preco)) $planos[$i]->nome = DB::select(DB::raw("SELECT descr FROM tabela_precos WHERE id = ".$consulta[$i]->id_plano))[0]->descr;
            else $planos[$i]->nome = "PLANO EXCLUÍDO";
            $planos[$i]->historico = DB::select(DB::raw("
                SELECT
                    descr,
                    max_atv,
                    max_atv_semana,
                    log_contrato.lixeira,
                    DATE_FORMAT(data_banco, '%m') AS mes

                FROM log_contrato

                WHERE id_banco = ".$consulta[$i]->id_plano."
                  AND data_banco >= '".$consulta[$i]->data."'
    ...
    public function atividades($id) {
        $consulta = DB::select(DB::raw("
            ...
            FROM (
                SELECT
                    pedido.id,
                    CONCAT('Contratação de ', tabela_precos.descr) AS acao,
                    pedido.data,
                    1 AS primeiro
                
                FROM pedido
                
                CROSS JOIN tabela_precos
                ...
                JOIN (
                    ...
                    WHERE id = ".$id."
                    GROUP BY id_pedido
                ) AS pp1 ON pp1.id_pedido = pedido.id

                WHERE tabela_precos.id IN (
                    SELECT id_plano
                    FROM pedido_planos
                    WHERE id = ".$id."
                )
                ...
                UNION ALL (
                    ...
                    JOIN (
                        ...
                        WHERE id = ".$id."
                        GROUP BY id_pedido
                    ) AS pp2 ON pp2.id_pedido = agenda.id_pedido

                    WHERE agenda.id_tabela_preco IN (
                        SELECT id_plano
                        FROM pedido_planos
                        WHERE id = ".$id."
                ...
                UNION ALL (
                    SELECT
                        congelamentos.id,
                        CASE
                            WHEN congelamentos.acao = 'C' THEN 'Congelamento'
                            ELSE 'Descongelamento'
                        END AS acao,
                        congelamentos.data,
                        0 AS primeiro
                    ...
                    JOIN (
                        ...
                        WHERE id = ".$id."
                        GROUP BY id_pedido
                    ) AS pp3 ON pp3.id_pedido = congelamentos.id_pedido
                )
            ) AS main
        ...
        foreach($consulta as $linha) {
            if ($linha->acao == "Agendamento") {
                $aux = DB::select(DB::raw("
                    SELECT 
                        UPPER(pessoa.nome_fantasia) AS profissional,
                        procedimento.id_especialidade,
                        DATE_FORMAT(agenda.created_at, '%d/%m/%Y') AS dtlan,
                        agenda.created_by,
                        agenda.obs_cancelamento,
                        agenda.status,
                        agenda.hora,
                    ...
                    WHERE agenda.id = ".$linha->id
                ))[0];
            } else $aux = new \stdClass;
            $aux->titulo = $linha->acao;
            $aux->titulo .= $linha->acao != "Agendamento" ? " em ".$linha->data : " - ".$aux->status;
