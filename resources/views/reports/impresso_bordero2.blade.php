...
<body class="A4">
    ...
    <table styleyle="display: flex;margin: 2px -20px 2px -20px;padding: 5px 16px 0px 16px;">
        <thead>
            <th>Contrato</th>
            <th>Modalidade</th>
            <th>Associado</th>
    ...
    @for($i = 0; $i < sizeof($agendamentos); $i++)
        @if ($agendamentos[0]->id_modalidade != 0)
            <td>{{ $agendamentos[$i]->id_contrato, 0, 37 }}</td>
            <td>{{ $agendamentos[$i]->descr_modalidade }}</td>
            <td>{{ $agendamentos[$i]->descr_pessoa }}</td>
            <td>{{ date('d/m/Y', strtotime($agendamentos[$i]->data)) }}</td>
            <td>{{ substr($agendamentos[$i]->hora,0, 5) }}</td>

            
            
            @if ($agendamentos[$i]->tipo_pagamento <> 'N' || 
            ucfirst(substr($formas_pag[$i],0,1)) . strtolower(substr(strtr($formas_pag[$i], $caracteres_sem_acento),1)) == 'Sem valor')
                @if(ucfirst(substr($formas_pag[$i],0,1)) . strtolower(substr(strtr($formas_pag[$i], $caracteres_sem_acento),1)) == 'Sem valor')
                    <td>SEM VALOR</td>
                    <td>0,00</td>
                    <td>0,00</td>
                    <td>0,00</td>
                ...
                @endif
            @else
                <td>{{ ucfirst(substr($formas_pag[$i],0,1)) . strtolower(substr(strtr($formas_pag[$i], $caracteres_sem_acento),1)) }}</td>
                @if ($agendamentos[$i]->tipo_de_comissao == '%')
                    <td>{{ $percentual_de_comissao[$i] }}%</td>
                @else 
                    <td>R$ {{ number_format($percentual_de_comissao[$i],2,",",".") }}</td>
                @endif
                    ...
                    <div style="display: flex">
                        <div style="display: flex;justify-content: space-between;font-size: 11px;width: 57%;">
                            ...
                            <div style="width: 79%;">
                                <span>
                                    <strong>
                                        @if (substr($agendamentos[$i]->descr_pessoa,0,23) != $agendamentos[$i]->descr_pessoa)
                                            {{ substr($agendamentos[$i]->descr_pessoa, 0, 23)}}
                                        @else 
                        ...
                        <div style="font-size: 12px;display: flex;justify-content: space-between;width: 43%;">
                            ...
                            <div style="font-size: 16px;margin-top: -5px;">
                                <span>
                                    <strong id="valores">
                                        @if (in_array($agendamentos[$i]->tipo_pagamento, array("E", "R", "C")) || ucfirst(substr($formas_pag[$i],0,1)) . strtolower(substr(strtr($formas_pag[$i], $caracteres_sem_acento),1)) == 'Sem valor')
                                            {{number_format(0,2,",",".")}}
                                        @else 
