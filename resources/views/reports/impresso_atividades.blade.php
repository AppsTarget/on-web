    ...
    <section class="sheet padding-10mm">
        <div class="row">
            ...
            <div class="report-title col d-grid">
                <h4 class="text-center mt-auto" style="color: #212529;line-height: 30px;">
                    <span style="font-size: 31px;">Contrato</span>
                    <br>
        ...
        <div class="report-header d-flex" style="margin-bottom:1rem; color: #212529;">
            <div class="header-left">
                ...
                <div class="resumo-info">
                    <span class="custom-label-form">Pagamento</span>
                    @if ($pedido_header->descr_convenio != "")
                        <h5>{{ $pedido_header->descr_convenio }}</h5>
                    @else 
                        <h5>Particular</h5>
                    @endif
                </div>
        ...
        <div class="row">
            ...
            <input type="hidden" value="{{ $plano = "" }}">
            <input type="hidden" value="{{ $planos_pag = 0 }}">
                @foreach($agendamentos AS $agendamento)
                    @if ($agendamento->descr_plano != $plano && $quebra != 6)
                        <div style="width: 100%;color: #212529;margin-top: 3%;margin-bottom: 1%;">
                            <h3 style="font-size: 20px">Plano: {{ucfirst($agendamento->descr_plano)}}</h3>
                        </div>
                        <input type="hidden" value="{{ $plano = $agendamento->descr_plano }}">
                        <input type="hidden" value="{{ $quebra = $quebra }}">
                    @endif
                    @if($quebra == 6)
                        </ul></div></section>
                        <section class="sheet padding-10mm">
                            ...
                            <div>
                                <p class="col p-0">
                                    <span class="ml-0 my-auto" style="font-weight:600">
                                            {{
                                                date('d/m/Y', strtotime($agendamento->data)) . ' ' . substr($agendamento->hora, 0, 5) . '  -  ' . strtoupper($agendamento->nome_profissional)
                                            }}
                                    </span>
                                </p>
                ...
                @endforeach
                <div style="display: block; width: 100%; color: black">
                    <p class="text-right">Total de Atividades: {{$total_atividades[0]->total}}</p>
                    <p class="text-right">Total de Atividades Realizadas: {{sizeof($agendamentos)}} </p>
                    <p class="text-right">Total de Atividades Restantes: {{$total_atividades[0]->total - sizeof($agendamentos)}}</p>
                </div>
            <ul>
        </div>
    </section>
