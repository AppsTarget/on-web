    ...
    @for($i = 0; $i < sizeof($agendamentos); $i++)
                ...
                <div class="row">
                    ...
                    <div style="width:100%;line-height: 10px;margin-top: -2%;margin-bottom: 2%;">
                        ...
                        <p style="text-align: center;color: #d20000;">
                            {{ getEmpresaObj()->descr}} - {{ getEmpresaObj()->cidade}}
                        </p>
                    ...
                    <div class="col-3" style="border: 2px solid;border-radius: 15px;min-height: 70px;">
                        ...
                        <div style="display: flex;justify-content: center;">
                            <p style="font-size: 14px;text-align: center;position: absolute;bottom: -10px;">
                                {{$data_ini->format('d/m/Y')}} à {{$data_fim->format('d/m/Y')}}
                            </p>
                        </div>
                    </div>
                    <div class="col-3" style="border: 2px solid;border-radius: 15px;min-height: 70px;">
        ...
        @endif
            ...
            @if($quebra == 12 and $i < 12)
                    ...
                    <p style="position: absolute;bottom: 55px;right: 45px;">
                    ...
                    <div style="display: flex">
                        <div style="display: flex;justify-content: space-between;font-size: 11px;width: 57%;">
                            ...
                            <div style="width: 79%;">
                                <span>
                                    <strong>
                                        @if (substr($agendamentos[$i]->descr_pessoa,0,40) != $agendamentos[$i]->descr_pessoa)
                                            {{ substr($agendamentos[$i]->descr_pessoa, 0, 40) . "..." }}
                                        @else 
                        ...
                        <div style="font-size: 12px;display: flex;justify-content: space-between;width: 43%;">
                            @if ($agendamentos[$i]->tipo_pagamento <> 'N')
                                ...
                                @if ($agendamentos[$i]->tipo_pagamento == 'E')
                            ...
                            @else
                                <div>
                                    @if ($agendamentos[$i]->tipo_de_comissao == '%')
                                        <span>%Comis...:<strong>{{ $percentual_de_comissao[$i] }}%</strong></span>
                                    @else 
                                        <span>Fixo...:<strong>R$ {{ number_format($percentual_de_comissao[$i],2,",",".") }}</strong></span>
                                    @endif
                                </div>
                            @endif
                            <div style="font-size: 16px;margin-top: -5px;">
                                <span>
                                    <strong id="valores">
                                        @if (in_array($agendamentos[$i]->tipo_pagamento, array("E", "R", "C")))
                                            {{number_format(0,2,",",".")}}
                                        @else 
        ...
        window.addEventListener('load', () =>{
            setTimeout(() => {
                ...
                document.querySelectorAll("#valores").forEach(el => {
                    valor_total += parseFloat(el.innerHTML)
                })
